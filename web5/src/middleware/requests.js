...
import d3request from 'd3-request/src/request.js';
import d3json from 'd3-request/src/json.js';
        ...
        return function(action)
        {
            switch(action.type)
            {
                ...
                case a.OVERLAY_SAVE:
                    ...
                    d3request('pavilions/'+overlay.id+'/overlay/')
                        .mimeType("application/json")
                ...
                case a.STAND_DELETE:
                    var stand = action.payload;
                    var stands_id = selectedStandsId(store);
                    d3request('stands/'+stands_id+'/'+stand.id+'|delete')
                      .mimeType("application/json")
                      .get(function(er, xhr){
                            if(er) store(a.ERROR_SET, er);
                            else {
